...
Slot files are written to disk after both calls succeed. Then chassis + slots are committed together as a single git repo and pushed.
...
2. Verification pass (deepseek-v3, t=0.3, verifier prompt)
   ↓
3. Build check       (next build, technical correctness)
   ↓
4. Retry only on:    malformed JSON OR build failure
```
...
- This is not a stillbirth. The app still ships.

### Failure mode 3: build failure
...
### Failure mode 4: build succeeds but app crashes at runtime
...
Per-app total LLM cost (guard + matcher + generator + verifier + readme): under $0.05.
...
4. Runs generator
5. Runs verifier
6. Verifies both outputs are valid JSON
7. Writes files alongside the chassis
8. Runs `next build` in a temp directory
9. Asserts build succeeds
