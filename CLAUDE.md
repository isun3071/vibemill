...
> **Changelog v6 (Bundle I):** Flask rail via `deploy_target=github_only`. Seven previously-stubbed archetypes (`glorified_todo`, `parody_ui`, `marketplace`, `map_visualizer`, `recommendation_engine`, `game`, `glorified_social`) become buildable as Flask apps that live as GitHub repos only — no Vercel, no HF Spaces. The GitHub repo IS the artifact; visitors clone and run locally. Genre-authentic to a huge slice of the real hackathon corpus that submits as "repo + README + demo video," never deployed. Database (SQLite, Postgres), auth (Flask-Login, OAuth), and file storage are now allowed in generated code because the mill never collects real user data — the app never runs on mill infrastructure. README setup steps are part of the artifact; tier drives DB use (slop=none, mean_good=optional, banger=required). `SUBSTRATE_BY_ARCHETYPE` values switched from `js`/`python` to stack names (`nextjs`/`gradio`/`flask`). New schema column `apps.substrate` (migration 011). All 13 archetypes from the Bundle F taxonomy are now buildable.
...
- **Code generator** (DeepSeek V4 Flash via OpenRouter, t=0.7) producing apps across the buildable archetype set (Bundle F: tracker, chatbot, utility_tool, search_directory; Bundle H adds ai_generator + ai_agent on the Gradio + HF Spaces rail; Bundle I adds glorified_todo, parody_ui, marketplace, map_visualizer, recommendation_engine, game, glorified_social on the Flask + github_only rail — all 13 archetypes now buildable)
